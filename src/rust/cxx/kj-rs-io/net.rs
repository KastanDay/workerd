...
impl TokioAddress {
    async fn parse(text: &str, port_hint: u16) -> Result<Self> {
        ...
        // Not a literal: resolve the hostname via getaddrinfo, which runs on tokio's blocking
        // pool with its completion forwarded through a runtime task so this await resumes
        // same-thread (a fast-path choice, not a soundness requirement). See `resolve_host`.
...
/// Resolves a hostname via blocking `getaddrinfo` on tokio's blocking pool. A tokio *runtime*
/// task owns the blocking `JoinHandle`, so the blocking-pool completion wakes tokio's own

...
class JsRpcClientProvider: public jsg::Object {
 public:
  ...
  struct ClientForOneCall {
    ...
    // Spans to parent the per-call jsRpcCall under. Root Fetchers use the newly-opened
...
class JsRpcStub: public JsRpcClientProvider {
 public:
  ...
  // These overloads accept an already-owned originatingCall because dup() may run without a current
  // IoContext; see JsRpcStub::dup().
  JsRpcStub(IoOwn<rpc::JsRpcTarget::Client> capnpClient,
      kj::Maybe<IoOwn<TraceContextParent>> originatingCall = kj::none)
  ...
  JsRpcStub(IoOwn<rpc::JsRpcTarget::Client> capnpClient,
      ...
      kj::Maybe<TraceContextParent> originatingCall);
  explicit JsRpcStub(
      uint channelNumber, kj::Maybe<IoOwn<TraceContextParent>> originatingCall = kj::none)
